...
internal class ONSPAudioPluginUpdater
{
    ...
    private static bool unityRunningInBatchmode;
    ...
    private static bool RenameSpatializerPluginToOld(string currentPluginPath)
    {
        if (File.Exists(currentPluginPath))
        {
            ...
            try
            {
                ...
                Debug.LogFormat("Spatializer plugin renamed: {0} to {1}", currentPluginPath, targetPluginPath);
                return true;
            }
            catch (Exception e)
            {
                Debug.LogWarningFormat("Unable to rename spatializer plugin: {0}, exception {1}", currentPluginPath,
                    e.Message);
                return false;
            }
        }
        ...
        if (File.Exists(newX86PluginPath) || File.Exists(newX64PluginPath))
        {
            ...
            else
            {
                ...
                else if (dialogResult == 2)
                {
                    ...
                    {
                        Debug.LogWarning("Exception happened when deleting new spatializer plugin: " + e.Message);
                    }
                }
            }
            ...
            if (userAcceptsUpdate)
            {
                ...
                if (File.Exists(newX86PluginPath))
                {
                    ...
                    try
                    {
                        ...
                        Debug.Log("path = " + curX86PluginPathRel);
                    ...
                    {
                        Debug.LogWarning("Unable to rename the new spatializer plugin: " + e.Message);
                    }
                }
                ...
                if (File.Exists(newX64PluginPath))
                {
                    ...
                    try
                    {
                        ...
                        Debug.Log("path = " + curX64PluginPathRel);

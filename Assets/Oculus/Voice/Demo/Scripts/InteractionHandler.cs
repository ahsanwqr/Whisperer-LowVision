...
public class InteractionHandler : MonoBehaviour
{
    ...
    private void OnRequestResponse(WitResponseNode response)
    {
        if (!showJson)
        {
            if (!string.IsNullOrEmpty(response["text"]))
            {
                ...
                // send response Text to voice Snap turn
                VoiceSnapTurn(response["text"]);
            }
            else
            {
                textArea.text = freshStateText;
    ...
    private void VoiceSnapTurn(string text)
    {
        // check if its left turn if yes call Voice Snap turn left
        if (text.ToString().ToLower().Contains("turn left"))
        {
            snapTurn.LeftSnapTurn();
        }
        // check if its left turn if yes call Voice Snap turn right
        else if (text.ToString().ToLower().Contains("turn right"))
        {
            snapTurn.RightSnapTurn();
            Debug.LogError("Snap Turn right");
        }
    }

...
{
    // Push/pop list
    private static readonly Stack<ReflectionSnapshot> snapshotList = new();
    ...
    {
        if (CheckForAudioListener(other.gameObject)) PushCurrentMixerShapshot();
    }
    ...
    {
        if (CheckForAudioListener(other.gameObject)) PopCurrentMixerSnapshot();
    }
